            ...
            <Link key={ticket.id} href={`/admin/tickets/${ticket.id}`}>
              <div className="bg-slate-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 hover:border-brand-400 dark:hover:border-gray-400 transition-colors">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-1.5 truncate">
                  ...
                  {(() => { const qs = (ticket as any).quotes ?? []; const latest = qs.filter((q:any)=>q.status!=='declined').sort((a:any,b:any)=>new Date(b.created_at).getTime()-new Date(a.created_at).getTime())[0]; return latest ? <span className="text-purple-500 dark:text-purple-400"> · Quoted: {formatDateTime(latest.created_at)}</span> : null })()}
                </p>
              </div>
            </Link>
          ))}
