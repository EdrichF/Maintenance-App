...
import { CheckCircle, XCircle, FileText, Image as ImageIcon, AlertTriangle } from 'lucide-react'
...
export function CompletionReviewCard({ completion }: Props) {
  ...
  async function review(status: 'approved' | 'rejected') {
    ...
    setLoading(null); setConfirming(false); setConfirmingApprove(false)
    setSelectedReason(''); setOtherReason(''); setReasonError('')
    router.refresh()
  }
  ...
  return (
    <div className={`rounded-xl border p-5 space-y-4 ${isPending ? 'bg-orange-50 dark:bg-orange-900/10 border-orange-200 dark:border-orange-800/40' : completion.status === 'approved' ? 'bg-green-50 dark:bg-green-900/10 border-green-200 dark:border-green-800/40' : 'bg-rose-50 dark:bg-rose-900/10 border-rose-200 dark:border-rose-800/40'}`}>
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          {completion.notes && <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">{completion.notes}</p>}
        </div>
        <span className={`shrink-0 text-xs font-medium px-2.5 py-1 rounded-full ${isPending ? 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400' : completion.status === 'approved' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400'}`}>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
            {completion.poc_urls.map((url, i) => (
              <a key={i} href={url} target="_blank" rel="noopener noreferrer">
                <img src={url} alt={`POC ${i + 1}`} className="w-full aspect-square object-cover rounded-lg border border-gray-200 dark:border-gray-700 hover:opacity-90 transition-opacity" />
              </a>
            ))}
          </div>
        </div>
      ...
      {isPending && !confirming && !confirmingApprove && (
        <div className="flex gap-2 pt-1">
          <Button onClick={() => setConfirmingApprove(true)} className="flex-1 bg-green-600 hover:bg-green-700 text-white" size="sm">
      ...
      {isPending && confirmingApprove && (
        <div className="rounded-lg border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/20 p-3 space-y-3">
          <p className="text-sm font-medium text-green-700 dark:text-green-400 flex items-center gap-2">
            <CheckCircle size={15} /> Confirm approval? This marks the ticket as <strong>Completed</strong>.
          </p>
          <div className="flex gap-2">
            <Button onClick={() => review('approved')} loading={loading === 'approve'} className="flex-1 bg-green-600 hover:bg-green-700 text-white" size="sm">Yes, Approve</Button>
            <Button onClick={() => setConfirmingApprove(false)} variant="secondary" size="sm" className="flex-1" disabled={loading !== null}>Cancel</Button>
          </div>
        </div>
      )}
      ...
      {isPending && confirming && (
        <div className="rounded-lg border border-rose-200 dark:border-rose-800 bg-rose-50 dark:bg-rose-900/20 p-3 space-y-3">
          ...
          <div className="flex gap-2">
            <Button onClick={() => review('rejected')} loading={loading === 'reject'} variant="danger" size="sm" className="flex-1">Confirm Reject</Button>
            <Button onClick={() => { setConfirming(false); setSelectedReason(''); setOtherReason(''); setReasonError('') }}
              variant="secondary" size="sm" className="flex-1" disabled={loading !== null}>Cancel</Button>
          </div>
        </div>
      )}
    </div>
